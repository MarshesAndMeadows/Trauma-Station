...
public sealed partial class DeepFryerSystem
{
    ...
    public void MakeCrispy(EntityUid item, ProtoId<CrispinessLevelSetPrototype> crispiness) // Frontier: add CrispinessLevelSetPrototype
    {
        ...
        if (_prototype.TryIndex<CrispinessLevelSetPrototype>(oldCrispinessProto, out var oldCrispiness))
        {
            _appearance.SetData(item, oldCrispiness.Visual, false);
        }
        if (_prototype.TryIndex<CrispinessLevelSetPrototype>(crispiness, out var newCrispiness))
        {
            _appearance.SetData(item, newCrispiness.Visual, true);
        }
        // End Frontier
    }
    ...
    public bool TryMakeMobIntoFood(EntityUid mob, MobStateComponent mobStateComponent, bool force = false)
    {
        ...
        if (force || _mobState.IsDead(mob, mobStateComponent))
        {
            ...
            if (!_solutionContainer.EnsureSolutionEntity(mob, mobFoodComponent.Solution, out var alreadyHadFood, out var mobFood))
                return false;
            ...
            if (TryComp<BloodstreamComponent>(mob, out var bloodstreamComponent) && bloodstreamComponent.ChemicalSolution != null)
            {
                ...
                mobFoodSolution.AddSolution(bloodstreamComponent.ChemicalSolution.Value.Comp.Solution, _prototype);
            }
            _solutionContainer.UpdateChemicals(mobFood.Value);
    ...
    private void MakeEdible(EntityUid uid, DeepFryerComponent component, EntityUid item, FixedPoint2 solutionQuantity)
    {
        ...
        if (!_solutionContainer.EnsureSolutionEntity(item, foodComponent.Solution, out var foodEnt))
            return;
        ...
        foodSolution.AddSolution(component.Solution.SplitSolution(solutionQuantity), _prototype);
        foodSolution.AddSolution(extraSolution, _prototype);
        _solutionContainer.UpdateChemicals(foodEnt.Value);
    }
}
